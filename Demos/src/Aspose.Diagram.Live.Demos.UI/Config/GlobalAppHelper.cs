...
namespace Aspose.Diagram.Live.Demos.UI.Config
{
    ...
    public class GlobalAppHelper
    {
		...
		public GlobalAppHelper(HttpContext hc, HttpApplicationState appState, string SessionID, string language)
		{
			...
			AsposeDiagramContext.atcc = new AsposeDiagramContext(hc); // sync the context
		...
		private void initResources(string ResourcesFile, string SessionID)
		{
			...
			if (AsposeDiagramContext.Current.Cache[SessionID] == null)
			{
				...
				AsposeDiagramContext.Current.Cache.Add(
				   SessionID,
					resources,
