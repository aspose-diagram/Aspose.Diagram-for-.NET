(function () {
	...
	app.factory('DocumentPagesFactory', function ($resource) {
        return $resource(apiURL + '/Viewer/DocumentPages?'+ 'file=' + fileName + '&folderName=' + folderName + '&pageCount=0&lstPages=null', {}, {
			query: {
				method: 'GET',
				isArray: false
			}
	...
	app.factory('myService', function ($http) {
        var getData = function (page) {
            return $http({
                method: "GET",
                url: apiURL + '/Viewer/DocumentPages?'+ 'file=' + fileName + '&folderName=' + folderName + '&currentPage=' + page,
		...
		function ViewerAPIController($scope, $sce, $http, $window, DocumentPagesFactory, myService, $loading, $timeout, $q, $alert) {
			...
			$scope.createPageImage = function (selectedFile, indx) {
				if (indx <= (imagedata.length - 1)) {
					...
					prevoiusIndx = indx;
					var imgData = $sce.trustAsResourceUrl(apiURL + '/Viewer/pageimage?imagePath=' + selectedFile);
