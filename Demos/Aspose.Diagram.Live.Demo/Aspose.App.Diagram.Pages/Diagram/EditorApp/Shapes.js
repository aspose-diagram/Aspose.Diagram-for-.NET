...
(function()
{
	...
	mxPerimeter.CalloutPerimeter = function (bounds, vertex, next, orthogonal)
	{
		return mxPerimeter.RectanglePerimeter(mxUtils.getDirectedBounds(bounds, new mxRectangle(0, 0, 0,
			Math.max(0, Math.min(bounds.height, parseFloat(
			CalloutShape.prototype.size)))),
			), vertex, next, orthogonal);
	};
	...
	if (typeof mxVertexHandler !== 'undefined')
	{
		...
		window.createHandle = createHandle;
		window.handleFactory = handleFactory;
	...
	else
	{
		...
		window.createHandle = function() {};
		window.handleFactory = {};
	}
